...
relationTypeColors:
  ...
  equivalent:     { light: "#10b981", dark: "#34d399" }

bulk_exports:                           # Restrict the dataset page's Downloads
  - csv                                # card (v0.7.135+). Absent/empty = all.
  - ttl
```
...
The 3D **relation sphere** visualizes a concept's neighborhood as an interactive force-directed sphere. Each surrounding node is a directly related concept, color-coded by relationship type. A degree filter lets users collapse the sphere to specific relationship categories (e.g. show only broader/narrower, or only cross-vocabulary matches); whole legend rows toggle their category on and off, and an info icon links to the relationship type's description. References that point outside the dataset (or are partially elided in source data) render as ghost nodes, so unresolved links are visible rather than silently dropped.
...
## Bulk exports (v0.7.135+)

Every build also emits single-file aggregate distributions per dataset into `public/data/{id}/`:
...
| `{id}.ttl` | Turtle | `dcat:Dataset` + `skos:ConceptScheme` metadata |

Files from the dataset's GCR `compiled/` directory (TBX, JSON-LD, YAML…) are copied alongside and listed too. The dataset page's **Downloads** card offers everything found, unless you restrict it with `bulk_exports` in `site-config.yml`:
