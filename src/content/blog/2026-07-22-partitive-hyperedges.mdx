...
> **Update 2026-07-23 — superseded by PartitiveRelation v2.**
> The design described in this post had three architectural problems
> that surfaced under review (misnamed per ISO 704/12620, conflated
> data semantics with diagram notation, missing criterion dimension).
> The v2 redesign renames `PartitiveHyperedge` → `PartitiveRelation`,
> promotes markers to structured data, adds per-partitive certainty
> and ExternalConcept support, and is **breaking**. See
> [PartitiveRelation v2 — ISO-aligned redesign](/blog/2026-07-23-partitive-relations-v2)
> for the current design and the
> [migration script](https://github.com/glossarist/concept-model/blob/main/exe/migrate-to-partitive-relation-v2)
> to upgrade. This post is preserved as the historical announcement.
...
`PartitiveHyperedge` captures all three on a single object. See the new [Partitive Relations](/model/partitive-relations) model page for the full field reference (now describing the v2 `PartitiveRelation` redesign that supersedes this announcement).
...
- [Partitive Relations](/model/partitive-relations) — full model reference (v2 redesign that supersedes this announcement)
