...
> **Update 2026-07-30 — naming follow-up.**
> The v3 model classes described here as `PartitiveRelation` / `GenericRelation`
> were renamed to `PartitiveHyperedge` / `GenericHyperedge` shortly after this
> post, with abstract bases `HyperedgeMember` and `AbstractHyperedge` and a
> `HyperedgeRegistry` for OCP-compliant type dispatch. The wire format and
> semantics are unchanged. See [Hyperedges](/model/hyperedges) for the current
> model page with SVG diagrams showing how the rakes look.
