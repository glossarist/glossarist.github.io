...
| `designations` | [Designation](/model/designations)[] | 0..* | Terms under which the concept is known |
...
| `sources` | [ConceptSource](/model/sources)[] | 0..* | Per-language sources |
...
| `sources` | [ConceptSource](/model/sources)[] | 0..* | Per-item sources |
...
See the [YAML Schema Reference](/reference/schema-browser) for the complete JSON Schema definitions, the [Entity Field Reference](/reference/entity-fields) for all entity types with types, cardinality, and allowed values, and the [Standards compliance reference](/reference/standards) for how each ISO standard maps to Glossarist entities.
