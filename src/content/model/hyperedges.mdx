...
The concept file no longer carries hyperedges inline. The directory IS the index — `relations/5-1/` enumerates every decomposition of concept 5.1.
...
The `type` field discriminates between `partitive_relation` and `generic_relation` on the wire. (The wire name retains `_relation` for backward-compatibility with the existing JSON Schema identifier; the in-code class name is `PartitiveHyperedge` / `GenericHyperedge`.)
