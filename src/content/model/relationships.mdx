...
Two new binary relationship types support placeholders for
concepts referenced but not defined in this dataset:

- `provides` — subject concept provides substance for an
  [ExternalConcept](/model/concepts#externalconcept)
- `provided_by` — subject ExternalConcept is provided by the object
  concept

Established at the collection level (whoever has both datasets
loaded). Distinct from `supersedes` / `superseded_by` — the
external concept stays; its substance just comes from elsewhere.

### Equivalence and mapping

```yaml
related:
  - type: equivalent         # Exact match in another vocabulary
    content: "iso-19107:entity"
  - type: broad_match
    content: "iso-19109:object"   # Broader concept in another vocabulary
  - type: close_match
    content: "3.1.1.1"            # Close match in same vocabulary
```
...
See [Designations](/model/designations#designation-relationships) for the full DesignationRelationship model. The `ref.text` field on concept-level relationships serves a different purpose — it provides designation text when the concept is identified by text rather than by registry reference.
...
| `equivalent` | `skos:exactMatch` | Within- and cross-vocabulary |
...
See the [YAML Schema Reference](/reference/schema-browser) for the JSON Schema definition of the RelatedConcept entity, the [Entity Field Reference](/reference/entity-fields) for field-level details, and the [Standards compliance reference](/reference/standards) for ISO standard mappings.
