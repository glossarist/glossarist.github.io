...
ISO 12620 / TBX `term_type` classification for [designations](/model/designations). Each designation can carry a `term_type` that classifies what kind of term it is — orthographic variants, symbolic notations, or usage/provenance categories.
...
See the [Entity Field Reference](/reference/entity-fields) for the complete enumeration with definitions, or the [Standards compliance reference](/reference/standards) for ISO standard mappings.
