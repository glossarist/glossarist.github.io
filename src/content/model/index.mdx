...
|---|---|
| **[Dataset register & sections](/model/datasets)** | Self-describing datasets with hierarchical sections, transitive membership, and ordering method (systematic / mixed / alphabetical) |
| **[Non-verbal entities](/model/non-verbal)** | Dataset-level `Figure`, `Table`, `Formula` with localized captions, alt text, image variants, and subfigures |
| **[Designation relationships](/model/designations#designation-relationships)** | First-class `DesignationRelationship` class for designation-level links (e.g. `abbreviated_form_for`) |
...
| **Inline mentions** | `ConceptSource#id` enables cite mentions in running text; fig mentions for figures (see [Sources](/model/sources#inline-citations)) |
...
- [Concepts](/model/concepts) — ManagedConcept, LocalizedConcept, concept lifecycle, collections
- [Designations](/model/designations) — Designation type hierarchy, pronunciation, base properties
- [Relationships](/model/relationships) — Typed relationship kinds across 4 standards
- [Sources](/model/sources) — Authoritative source hierarchy and provenance
- [Datasets & sections](/model/datasets) — DatasetRegister, hierarchical sections, ordering, bibliography
- [Non-verbal entities](/model/non-verbal) — Figure, Table, Formula as dataset-level resources
- [Term Types](/model/term-types) — ISO 12620 term type classifications
- [YAML Schemas](/model/schemas/) — V2 and V3 schema overview and examples
