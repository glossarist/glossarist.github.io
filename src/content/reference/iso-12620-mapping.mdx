...
| cross-vocabulary equivalence | `ManagedConcept.related` with SKOS types | `equivalent`, `close_match`, `broad_match`, `narrow_match`, `related_match` — see [Relationships — SKOS Alignment](/model/relationships#skos-alignment) |
...
| sequential (`A.6.x`) | `related: { type: sequentially_related_concept }` | |
