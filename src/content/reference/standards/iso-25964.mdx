...
<figure>
  <img src="/images/model/Concepts.png" alt="UML class diagram of the Glossarist Concept model showing the relationship hierarchy" />
  <figcaption><strong>Concept model hierarchy.</strong> ManagedConcept and LocalizedConcept types, their relationship collections, and how typed semantic links (broader/narrower with generic, partitive, and instantial subtypes) connect concepts into navigable systems.</figcaption>
</figure>
...
|---------------|-----------------|---------|
| `skos:exactMatch` | `equivalent` | Concepts are exactly equivalent |
...
Many relationship types are directional — `broader` in one direction implies `narrower` in the reverse. The [concept-browser](/docs/software/concept-browser) derives these inverse edges automatically, so a relationship need only be declared in one direction.
